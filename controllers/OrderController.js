...
const orderController = {
  ...
  async createOrderForSingleProduct(req, res) {
    const { userId, productId, quantity, shippingAddress, paymentMethod } =
      req.body;
    ...
    if (
      ...
      logError("createOrderForSingleProduct", "Missing required fields", {
        userId,
        productId,
      });
      return res.status(400).json({
        success: false,
        message:
          "User ID, productId, quantity, shippingAddress, and paymentMethod are required",
      });
    }
    ...
    try {
      ...
      logError("createOrderForSingleProduct", error.message, {
        userId,
        productId,
      });
      res.status(500).json({
  ...
  async updateProductOrderStatus(req, res) {
    try {
      ...
      return res
        .status(
          error.message.includes("not found" || "not authorized") ? 404 : 500
        )
        .json({
          success: false,
  ...
  async updateProductSellerOrderStatus(req, res) {
    try {
      const { id } = req.params;
      ...
      const updatedOrder =
        await SellerOrderService.updateProductSellerOrderStatus(id, status);
      return res.status(200).json({
        success: true,
        message: "Product order status updated successfully",
      });
