...
class ProductController {
  ...
  async acceptProduct(req, res) {
    try {
      const { productId } = req.params;
      const product = await ProductService.updateVerificationStatus(
        productId,
        true
      );
      ...
      return res
        .status(500)
        .json({ message: "Error accepting product", error: error.message });
    }
  }
  ...
  async getSellerProductsStockLevels(req, res) {
    try {
      ...
      const stockLevels = await ProductService.getSellerProductsStockLevels(
        sellerId
      );
  ...
  async rejectProduct(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Product rejected successfully", product });
    } catch (error) {
      return res
        .status(500)
